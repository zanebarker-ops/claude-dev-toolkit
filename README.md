...
|----------|-------|-------------|
| **Hooks** | 14 | Event-driven scripts that enforce workflow rules |
| **Hookify Rules** | 11 | Markdown-based rules for blocking/warning on patterns |
| **Agent Commands** | 23 | Specialized AI agent prompts (`/start-task`, `/security-auditor`, etc.) |
| **PR Review Toolkit** | 6 agents | Automated multi-agent code review before PRs |
| **Scripts** | 3 | Lint runner, deploy checker, session manager |
| **Templates** | 6 | CLAUDE.md, worktree workflow, model selection, agents, PRP, hookify docs |
...
```bash
# Option A: Clone into WSL filesystem (faster I/O, recommended for large repos)
cd ~
mkdir repos && cd repos
git clone git@github.com:your-org/your-project.git

# Option B: Clone into Windows filesystem (accessible from Windows Explorer)
cd /mnt/c/repos
git clone git@github.com:your-org/your-project.git
```

**Tradeoff:** WSL filesystem (`~/repos/`) has 5-10x faster I/O but files aren't visible in Windows Explorer. Windows filesystem (`/mnt/c/repos/`) is slower but accessible from both sides.
