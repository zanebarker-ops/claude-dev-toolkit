...
<div class="eyebrow mb-4"><carbon:bare-metal-server-01 /> 15 sticky tabs</div>

# Two layers of tmux. Zero lost work.

<div class="grid grid-cols-2 gap-4 mt-4">

<div class="hud" style="border-color:#22d3ee;">
<div class="kicker cyan mb-2">outer · terminal-tab layer</div>
<h3 class="text-base mb-2"><code>~/scripts/wsl-session.sh</code></h3>
<p class="text-xs muted mb-2">Windows Terminal exposes 15 named profiles. Each profile invokes <code>tmux new-session -A -s &lt;name&gt;</code> — attaches if it exists, creates if it doesn't.</p>
<div class="kicker dim">closing the tab does not kill the work</div>
</div>

<div class="hud hud-magenta">
<div class="kicker magenta mb-2">inner · claude-session layer</div>
<h3 class="text-base mb-2"><code>scripts/claude-session.sh</code></h3>
<p class="text-xs muted mb-2">Inside any tab, this launches Claude Code in its own tmux session for the current repo/worktree. Project-aware: detects branch, beads, prereqs.</p>
<div class="kicker dim">survives VS Code crashes</div>
</div>

</div>

```jsonc
// Windows Terminal settings.json — one profile per session, ×15
{ "name": "Session - Main",
  "commandline": "wsl.exe -d Ubuntu -- bash -lc \"$HOME/scripts/wsl-session.sh main\"" }
{ "name": "Session 2",
  "commandline": "wsl.exe -d Ubuntu -- bash -lc \"$HOME/scripts/wsl-session.sh 2\"" }
// ...Session 3 → Session 15
```

<p class="text-center text-sm cyan glow-cyan mt-3">
Main → default branch · 2-5 → active worktrees · 6-10 → dev servers, MCPs, dashboards · 11-15 → throwaway
</p>

<div class="status-bar">
  <span class="left">RUNBOOK · docs/wsl-tmux-terminal-setup.md</span>
  <span class="right">19 / 43</span>
</div>

<!--
- Outer tmux = the tab. Inner tmux = the Claude session. Detach with Ctrl-b d.
- Reopening a closed Windows Terminal tab re-attaches the same tmux session — scrollback, processes, Claude all intact.
- The 15 GUIDs in the runbook are pre-generated; settings.json is portable across machines.
- Pair this with claude-session.sh inside any tab to get crash-proof Claude on top of crash-proof tabs.
-->
